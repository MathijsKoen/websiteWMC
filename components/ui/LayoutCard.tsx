    ...
    <>
      ...
      {mounted && createPortal(
        <motion.div
          ...
          className="fixed inset-0 z-[100] bg-black/60 flex items-center justify-center p-6 cursor-pointer"
          onClick={() => setOpen(false)}
        >
          {/* Witte box — stopPropagation zodat klikken erin de modal niet sluit */}
          <motion.div
            initial={false}
            animate={{ scale: open ? 1 : 0.95, y: open ? 0 : 20 }}
            transition={{ duration: 0.25, ease: 'easeOut' }}
            className="bg-white w-full max-w-2xl h-full max-h-[95vh] overflow-y-auto shadow-2xl flex flex-col cursor-default"
